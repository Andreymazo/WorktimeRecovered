...
class Timesheet(models.Model):
    ...
    def save(
        ...
        if not self.status_work and self.datetime_start and self.datetime_complete:# Есть начало и конец рабочего дня статус нерабочий
            # Привязываем timesheet - foreign key , остальное ставим нули

            WorkTime.objects.create(timesheet_id=self.id, start_break_safe_sheets = str(self.datetime_start), end_break_safe_sheets = str(self.datetime_start), time_worked_per_day = str(self.datetime_start-self.datetime_start))
        
        if self.status_work and self.datetime_start and self.datetime_complete:# Есть начало и конец рабочего дня статус рабочий
            # Привязываем timesheet - foreign key, остальное ставим нули
            WorkTime.objects.create(timesheet_id=self.id, start_break_safe_sheets = 0, end_break_safe_sheets = 0, time_worked_per_day = 0)
        
        ## Остальное пока комментируем
